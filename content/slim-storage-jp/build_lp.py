...
ASINS = {
    ...
    "slim-storage-7":  "B0BCQ9GV7Z",   # Goowin 幅12cm キャスター付きスリムワゴン
    "slim-storage-8":  "B012VR26ME",   # JEJ スリムチェスト SIU-5-WH
    "slim-storage-9":  "B085F69484",   # 天馬 スキピタストッカースリム5段 17x140cm
    "slim-storage-10": "B0BX64K9J3",   # SVOHZAV スリムワゴン 18cm 四段式 ホワイト
}
...
"slim-storage-3": dict(
  ...
  why=["白い樹脂のワゴンは便利ですが、キッチンに置くと「収納用品」の顔をします。"
       ...
       "TORIBIO の13cm・5段は、黒いスチールフレームの上に木目の天板が乗った形です。"
       "上に置いたコーヒーミルやマグが様になり、収納として使いながら、いちばん上だけは"
       "「見せる場所」にできる——狭い部屋ではこの二役がよく効きます。"],
  ...
  card_body="木目の天板が付いた幅13cm・5段のスリムワゴン。中は普通に収納として使いながら、"
            "上だけはコーヒー道具やグリーンを置く場所にできます。生活感を増やさずに収納を増やしたい人向け。",
  alt="木目の天板と黒いフレームの細いワゴンが冷蔵庫の横に置かれたキッチン",
...
"slim-storage-6": dict(
  ...
  why=["直線、ロゴなし、白と木のツートン。tower シリーズの見た目のルールはそれだけです。"
       "でも狭い部屋では、この「主張しない」という性質が効きます。"
       "物が増えても、視界の情報量が増えないからです。",
       "幅13cm・奥行47.5cm・高さ80.5cm（メーカー表記）のスリムワゴンは、そのルールのまま隙間に入る一台です。"
       "本体は完成品でキャスターを付けるだけなので、届いたその日から使えます。"
  ...
  fit=["13cmは狭いので、<strong>入れる物のほうを先に決める</strong>のが失敗しないコツです。"
       ...
       "それが各段に収まるかで判断してください。",
       "奥行き47.5cmは意外と深いので、幅だけでなく<strong>奥行きも測ってください</strong>。"
       "冷蔵庫や洗濯機より前に出ると、通路の邪魔になります。"],
  ...
  card_body="白い本体に木の天板が乗った、幅13cmのスリムワゴン。収納を足しても視界の情報量が増えないので、"
            "狭い部屋ほど効きます。本体は完成品なので、届いたらキャスターを付けるだけで使えます。",
  alt="白い本体に木の天板が付いたスリムワゴンがキッチンの細い隙間に収まっている様子",
...
"slim-storage-7": dict(
  ...
  why=["洗面台の上は平面です。平面に物を並べると、置ける量はすぐ頭打ちになり、"
       "そこから先はただ散らかっていきます。",
       "幅12cmのキャスター付きワゴンは、その平面の横に縦の置き場所を足す形です。"
       "毎日使うスキンケアとヘアケアを段ごとに分けられるので、"
       "洗面台の上は「今使っている物だけ」に戻せます。キャスターで引き出せるため、"
       "掃除のときに全部どかす必要もなくなります。"],
  fit=["洗面台の横は、<strong>幅より高さと奥行きの制約</strong>が強い場所です。"
       "12cmという幅はかなり細いので入る隙間は多いのですが、"
       "洗面台より前に出ると、しゃがんだときや扉を開けたときに当たります。"
       "洗面台の奥行き以内に収まるかを先に確認してください。",
       "入れる物のほうから決めるのも手です。化粧水のボトル、ドライヤー、"
       "詰め替えパック——いちばん大きい物の幅と高さを測って、各段に収まるかで判断すると外しません。"],
  rent=["置くだけなので穴あけは不要です。突っ張り式や壁付け式は収納力が上がりますが、"
        "賃貸では跡が残らないタイプかを必ず確認してください。",
        "洗面所は湿気が多いので、水がかかったら拭く、という一手間だけ習慣にしておくと長持ちします。"
        "キャスター付きなら掃除のときに引き出せるので、裏側に水垢がたまりにくいのも利点です。"],
  card_h2="洗面台の「横」に、縦の置き場所を足す",
  card_body="幅12cmのキャスター付きスリムワゴン。洗面台の脇に差し込んで、"
            "スキンケアやヘアケアを段ごとに分けられます。洗面台の上が空くので朝の動線が短くなり、"
            "引き出せるぶん掃除もしやすくなります。",
  alt="洗面台の横の細い隙間に収まった幅12cmのキャスター付きスリムワゴン",
...
TITLE_OVERRIDE = {
    ...
    # 実際にリンクする商品に合わせて差し替え（tower→Goowin 幅12cm）
    "slim-storage-7": "洗面台のごちゃつきが消えた｜幅12cmのワゴンで美容収納",
}

# CSVの h2_1（「〇〇を選んだ理由」）を、実際にリンクする商品に合わせて差し替える
H2_OVERRIDE = {
    "slim-storage-2": "13cm幅のトイレラックを選んだ理由",
    "slim-storage-3": "木天板つきの13cmワゴンを選んだ理由",
    "slim-storage-7": "幅12cmのキャスター付きワゴンを選んだ理由",
}

PRODUCT_OVERRIDE = {
    "slim-storage-2": "山崎実業 ハンドル付きスリムトイレラック 13cm",
    "slim-storage-3": "TORIBIO スリムワゴン 13cm 5段 天板付き",
    "slim-storage-7": "Goowin スリムワゴン 幅12cm キャスター付き",
}
